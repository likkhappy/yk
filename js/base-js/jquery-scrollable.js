 ...
 */
    ...
    Scrollable.prototype = {
        ...
        _init: function () {
            ...
            var $width = container.get(0).getBoundingClientRect().width || container.get(0).offsetWidth;

            //if (options.width === "auto") options.width = $element.width();
            if (options.width === "auto") options.width =$width;
            ...
            for(var i=0;i<count;i++){
                var slide =  $content.eq(i);
                slide.css('width',options.width);
                slide.attr('data-index',i)
                var page = '<a href="javascriipt:void(0)" data-index="'+i+'"></a>';
                if(i == options.showindex){
                    page = '<a class="active" href="javascriipt:void(0)" data-index="'+i+'"></a>';
                }
            ...
            _this.pagelist.delegate(options.pageitem,'click',function(e){
                var ele = $(this);
                var index = ele.data('index');
                _this.to(index);
                _this.pagelist.find('a').removeClass('active');
                ele.addClass('active')
            });
        ...
        _run: function (type, callback) {
            ...
            _this.pagelist.find('a').removeClass('active');
            _this.pagelist.find('a[data-index='+_this.showIndex+']').addClass('active');
        },
